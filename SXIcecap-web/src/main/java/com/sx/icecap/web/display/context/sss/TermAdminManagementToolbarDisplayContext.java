...
import com.sx.icecap.web.security.permission.resource.sss.TermResourcePermissionHelper;
import com.sx.icecap.web.taglib.clay.sss.TermVerticalCard;
...
public class TermAdminManagementToolbarDisplayContext 
	...
	public TermAdminManagementToolbarDisplayContext(
		...
		_eventName = ParamUtil.getString(
				...
				_namespace + StationXWebKeys.SELECT_TERM);
					...
					new DropdownItemList() {
						{
							...
							add(
								dropdownItem -> {
									...
									dropdownItem.setLabel(LanguageUtil.get(request, "delete-selected-terms"));
									dropdownItem.setQuickAction(true);
									dropdownItem.putData("cmd", IcecapActionKeys.DELETE_TERMS);
								});
						}
					};
...
//		Debug.printFooter("TermAdminManagementToolbarDisplayContext.getActionDropdownItems()");
	...
	public List<String> getAvailableActions( Term term ) throws PortalException{
		...
		if (TermModelPermissionHelper.contains(
			...
			availableActions.add(IcecapActionKeys.DELETE_TERM);
		}
		
		if (TermModelPermissionHelper.contains(
				permissionChecker, term, IcecapActionKeys.UPDATE_TERM)) {

			availableActions.add(IcecapActionKeys.UPDATE_TERM);
		}
		
		if (TermModelPermissionHelper.contains(
				permissionChecker, term, IcecapActionKeys.ADD_TERM)) {

			availableActions.add(IcecapActionKeys.ADD_TERM);
		}

		if (TermModelPermissionHelper.contains(
				permissionChecker, term, IcecapActionKeys.REVIEW_TERM)) {

			availableActions.add(IcecapActionKeys.REVIEW_TERM);
		}

		if (TermModelPermissionHelper.contains(
				permissionChecker, term, IcecapActionKeys.APPROVE_TERM)) {

			availableActions.add(IcecapActionKeys.APPROVE_TERM);
		}
